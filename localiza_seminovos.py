...
class dashboard_localiza_movida:
    def __init__(self, caminho_db):
        self.conn = sqlite3.connect(caminho_db)
        self.df = None
        self.modelos_escolhidos_localiza = [...]
        self.modelos_escolhidos_movida = [...]

    def conectar_df(self):
        import time
        st.toast("🔌 Conectando ao banco de dados...")
        bar = st.progress(0, text="Iniciando carregamento do banco...")
        start = time.time()

        # Etapa 1: conexão (já foi feita no __init__)
        bar.progress(20, text="Banco de dados conectado.")

        # Etapa 2: leitura SQL
        bar.progress(40, text="Lendo dados da tabela 'cars'...")
        df_base = pd.read_sql_query("SELECT * FROM cars", self.conn)

        bar.progress(60, text=f"Convertendo coluna de datas...")
        df_base["Data"] = pd.to_datetime(df_base["Data"], errors="coerce")

        bar.progress(80, text="Fechando conexão...")
        self.conn.close()

        bar.progress(100, text=f"✅ Base carregada em {round(time.time() - start, 2)}s!")
        st.toast("✅ Leitura do SQL concluída!")
